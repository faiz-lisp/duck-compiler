...
(define (asm-compile-exp exp name)
  (let
    ( ;(asm (format "`which  nasm` ~a.s -f macho && ld -macosx_version_min 10.6 -arch i386 -e _start -no_pie -lc ~a.o -o ~a" name name name))
      [asm (format "`which  nasm` ~a.s -f macho" name)]
      (link
        (format "ld -macosx_version_min 10.6 -arch i386 -e _start -no_pie -lc ~a.o -o ~a" name name)
    ) )
    (printf "~a\n" asm)
    (system        asm)    
    (printf "~a\n" link)
